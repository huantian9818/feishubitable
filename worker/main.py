...
def run_once(client=None) -> bool:
    processed = run_worker_cycle(client=client)
    LOGGER.info("worker cycle finished; processed_job=%s", processed)
    return processed


def run_forever(interval_seconds: float = 5.0, client=None) -> int:
    LOGGER.info("worker loop started; interval_seconds=%s", interval_seconds)
    try:
        while True:
            should_sleep = True
            try:
                processed = run_once(client=client)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.once:
        run_once(client=resolved_client)
        return 0

    start_event_listener(client=resolved_client)
    return run_forever(interval_seconds=args.interval, client=resolved_client)
